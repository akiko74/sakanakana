class PicturesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @picture.save
        unless @divelog = Divelog.order('start_date DESC').where('start_date < ? and end_date > ?', @picture.exif.shot_date_time, @picture.exif.shot_date_time).first
          raise "#{Divelog.model_name.human} is not found..."
        end
